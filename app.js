...
/**
 * Smart Weather Forecast - User Panel Script
 * Manages weather reports fetching, filtering, and submissions.
 */

const API_URL = 'http://localhost:3000/weather';

// Cache for loaded weather logs
let weatherRecords = [];
...
/**
 * 2. Form Input Inline Error Resets
 */
const inputs = [cityName, cityTemp, cityCondition, cityHumidity, cityWindSpeed, cityDate];
...
function validateForm() {
  ...
  // City: Required and at least 3 chars
  ...
  // Temp: Required and between -50 and 60
  ...
  // Humidity: Required and between 0 and 100
...
function applyFilters() {
  ...
  const filtered = weatherRecords.filter(rec => {
    return (selectedCondition === 'all' || rec.condition === selectedCondition);
  });
...
/**
 * 6. Render Dynamic Weather Cards
 */
function renderCards(list) {
  if (list.length === 0) {
    weatherCardsContainer.innerHTML = `
      <div class="empty-state">
        <p style="font-size: 1.25rem; margin-bottom: 0.5rem;">⛅ No city weather reports found</p>
        <p>Try changing your active filter selection or submit a weather report.</p>
      </div>
    `;
    return;
  }
      ...
      <article class="weather-card" data-id="${rec.id}">
        <div class="weather-details">
          <div class="weather-header">
            ...
            <span class="badge badge-${rec.condition}">${rec.condition}</span>
          </div>
  ...
  weatherCardsContainer.innerHTML = `<div class="weather-grid">${cardsHTML}</div>`;
}

/**
 * 7. Submit Weather Report (POST)
 */
...
/**
 * Loading & Error Elements Helpers
 */
...
function showErrorState() {
  weatherCardsContainer.innerHTML = `
    <div class="error-state">
      ...
      <p>Failed to connect to the database. Make sure your server is running by running:</p>
      ...
      <button class="btn btn-secondary btn-sm" id="btnRetryLoad" style="margin-top: 0.5rem;">
        🔄 Retry Connection
      </button>
    </div>
  `;
